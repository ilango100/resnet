...
def plain_wocs(inputs: tf.Tensor, filters: int, reduce=False, momentum=0.9) -> tf.Tensor:
    stride = 2 if reduce else 1
    x = Conv2D(filters, 3, stride, "same")(inputs)
    x = BatchNormalization(momentum=momentum, center=False, scale=False)(x)
    x = Activation("relu")(x)
...
def resblock(inputs: tf.Tensor, filters:int, reduce=False, momentum=0.9) -> tf.Tensor:
    stride = 2 if reduce else 1
...
def resblock_prebn(inputs: tf.Tensor, filters: int, reduce=False, momentum=0.9) -> tf.Tensor:
    stride = 2 if reduce else 1
    x = BatchNormalization(momentum=momentum)(inputs)
    x = Conv2D(
        filters, 3, stride, "same", activation="relu")(x)
...
def resblock_prebn_wocs(inputs: tf.Tensor, filters: int, reduce=False, momentum=0.9) -> tf.Tensor:
    stride = 2 if reduce else 1
    x = BatchNormalization(
        momentum=momentum, center=False, scale=False)(inputs)
    x = Conv2D(
...
__all__ = ["plain", "plain_wocs", "plain_prebn", "plain_prebn_wocs", "resblock", "resblock_prebn", "resblock_prebn_wocs"]
